...
-   **核心下载** — 内置 Paper / Purpur / Fabric / Vanilla 等核心的版本列表与下载
-   **Java 运行时管理** — 自动检测架构并下载适配的 JRE（ARM64 v7a / v8a），支持国内镜像加速
-   **实时控制台** — 终端风格控制台，支持搜索/过滤、日志级别高亮、快速命令面板、复制日志
-   **在线玩家** — 自动解析日志，实时显示在线人数与玩家名
-   **性能监控** — 实时 CPU、内存、TPS/MSPT、玩家数、运行时间（仿 Pterodactyl 资源面板）
-   **启动健康检查** — 端口占用检测、JAR 完整性验证、内存/磁盘检查、Java 兼容性（仿 MCSManager 诊断）
